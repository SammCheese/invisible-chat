  ...
  render() {
    ...
    return (
      <Modal className="powercord-text">
        ...
        <Modal.Content>
          <TextAreaInput disabled={true} value={this.props.content} rows={1}>
            Secret Message
          </TextAreaInput>
          <TextAreaInput
            value={
          ...
          {!!userPws && (
            <SelectInput
              ...
              value={
                this.state.user
                  ? this.state.user
                  : (this.state.user = currUser.password)
              }
              onChange={async (e) => {
                await this.setState({ password: e.value });
                await this.setState({ user: e.label });
              }}
              rows={1}
            >
          ...
          {!!userPws && <div style={{ height: "20px" }}></div>}
          {!userPws && (
            <h3 style={{ color: "white" }}>
              Tip: You can set Passwords for Specific Users in the Plugin
              Settings
            </h3>
          )}
          {errorElement}
              ...
              try {
                ...
                if (Decrypted.match(UrlRegex)) {
                  return (
                    chatembed.executor(
                      Decrypted,
                      Decrypted.match(UrlRegex).toString()
                    ),
                    closeModal()
                  );
                }
